...
func getConnectorConfig(podName string, crName string, connectorName string, g Gomega) map[string]string {
	...
	connectors := rootMap["value"].(string)
	...
	for _, v := range listOfConnectors {
		...
		fmt.Printf("=======looking for connector %s\n", connectorName)
		fmt.Printf("======== connector: %s\n", v.Name)
		fmt.Printf("======== connector: %v\n", v.Params)
		if v.Name == connectorName {
			fmt.Printf("=======return= connector: %v\n", v.Params)
			return v.Params
		} else {
			fmt.Printf("xxxxxx not |%s| |%s|\n", v.Name, connectorName)
		}
	}
	return nil
}
...
func testConfiguredWithCertAndBundle(certSecret string, caSecret string) {
	...
	brokerCr, createdBrokerCr := DeployCustomBroker(defaultNamespace, func(candidate *brokerv1beta1.ActiveMQArtemis) {
		...
		candidate.Spec.DeploymentPlan.Image = "quay.io/hgao/amq-openshift-image:pem-1.0"
		candidate.Spec.DeploymentPlan.InitImage = "quay.io/hgao/init-container:pem-1.0"
	...
	Eventually(func(g Gomega) {
		...
		g.Expect(connectorCfg["keyStorePath"]).To(Equal(certSecret + ".pemcfg"))
