...
func checkStringSpecial(prop *string, specials map[string]string) *string {
	if nil == prop {
		...
		fmt.Println("mmmm the value is a special: " + *prop)
		uniqueKey := getUUID()
...
func MakeLoggingCfgOverrides(customResource *v2alpha5.ActiveMQArtemis) (string, map[string]string) {
	var sb, sbroot strings.Builder
	var specials map[string]string = make(map[string]string)

	loggers := customResource.Spec.Logging.Logger
	var hasRoot bool = false
	var loggingHead bool = false

	if len(loggers) > 0 {
		var loggersHead bool = false
		for _, logger := range loggers {
			if logger.Name == "" {
				//root logger
				if value := checkStringSpecial(logger.Level, specials); value != nil {
					sbroot.WriteString("log_level_root: " + *value + "\n")
					hasRoot = true
				}
				if value := checkStringSpecial(logger.Handlers, specials); value != nil {
					sbroot.WriteString("log_handlers_root: " + *value + "\n")
					hasRoot = true
				}
			} else {
				//normal logger
				...
				sb.WriteString("  - name: " + logger.Name + "\n")
				if value := checkStringSpecial(logger.Level, specials); value != nil {
					sb.WriteString("    level: " + *value + "\n")
				}
				if value := checkStringSpecial(logger.Handlers, specials); value != nil {
					sb.WriteString("    handlers: " + *value + "\n")
				}
				if value := checkBool(logger.UseParentHandlers); value != nil {
					sb.WriteString("    useParentHandlers: " + *value + "\n")
				}
			}
		}
	}
	//handlers
	handlers := customResource.Spec.Logging.Handler
	if len(handlers) > 0 {
		if !loggingHead {
			sb.WriteString("logging:" + "\n")
			loggingHead = true
		...
		for _, handler := range handlers {
			sb.WriteString("  - name: " + handler.Name + "\n")
			if value := checkStringSpecial(handler.ClassName, specials); value != nil {
				sb.WriteString("    className: " + *value + "\n")
			}
			if len(handler.Properties) > 0 {
				sb.WriteString("    properties:\n")
				for _, prop := range handler.Properties {
					if k := checkStringSpecial(topointer(prop.Name), specials); k != nil {
						sb.WriteString("    - key: " + *k + "\n")
					}
					if v := checkStringSpecial(topointer(prop.Value), specials); v != nil {
	...
	//formatters
	formatters := customResource.Spec.Logging.Formatter
	if len(formatters) > 0 {
		if !loggingHead {
			sb.WriteString("logging:" + "\n")
			loggingHead = true
		...
		for _, formatter := range formatters {
			if value := checkStringSpecial(topointer(formatter.Name), specials); value != nil {
				sb.WriteString("  - name: " + *value + "\n")
			}
			if value := checkStringSpecial(formatter.ClassName, specials); value != nil {
				sb.WriteString("    className: " + *value + "\n")
			}
			if len(formatter.Properties) > 0 {
				sb.WriteString("    properties:\n")
				for _, prop := range formatter.Properties {
					if k := checkStringSpecial(topointer(prop.Name), specials); k != nil {
						sb.WriteString("    - key: " + *k + "\n")
					}
					if v := checkStringSpecial(topointer(prop.Value), specials); v != nil {
	...
	if hasRoot {
		sb.WriteString("\n")
		sb.WriteString(sbroot.String())
	}
	return sb.String(), specials
}
